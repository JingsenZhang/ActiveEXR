...
class MLP(nn.Module):
    ...
    def forward(self, hidden):  # (batch_size, emsize)
        ...
        rating = torch.squeeze(self.linear2(mlp_vector), -1)  # (batch_size,)
        return rating
...
class PETER(nn.Module):
    def __init__(self, config):
        ...
        self.pos_encoder = PositionalEncoding(config['emsize'], config['dropout_prob'])  # emsize: word embedding size
        encoder_layers = TransformerEncoderLayer(config['emsize'], config['nhead'], config['hidden_size'], config[
            'dropout_prob'])  # nhid: dim_feedforward, one basic layer, including multi-head attention and FFN
        self.transformer_encoder = TransformerEncoder(encoder_layers, config['nlayers'])  # loop over the one above
        self.user_embeddings = nn.Embedding(config['nuser'], config['emsize'])
        self.item_embeddings = nn.Embedding(config['nitem'], config['emsize'])
        self.word_embeddings = nn.Embedding(config['ntoken'], config['emsize'])  # word emb
        self.hidden2token = nn.Linear(config['emsize'], config['ntoken'])  # the probability of each word
        self.recommender = MLP(config['emsize'])
